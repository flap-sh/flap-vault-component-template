...
Standard 3D uses the versioned `three-r3f-v1` profile on either a mode-less 7777 Vault UI or the existing token-scoped 8888 Mini App surface. The 7777 surface accepts factory, single-Vault, or token bindings, requires only 7777 proof tokens, stays in the default Vault shell, and keeps host risk status before the 3D visual without Mini App audio/full-height/displayTitle exemptions. The profile pins the supported Three/R3F packages, permits recursively statically reachable local source/shader/3D assets, requires every local file to be imported from the `Component.tsx` graph, requires deterministic renderer state/fallback attributes, and keeps network, wallet, storage, navigation, workers, remote assets, and arbitrary npm packages blocked. See `docs/mini-app-3d.md`; the shared homepage gallery exposes Butterfly Farm, Flap Energy Arena, Flap Skies, and the 7777 `three-r3f-example` fixture before the mode tabs.

For `three-r3f-v1` scaffolds, `--token` is optional when the project has no test token: the scaffold selects Flap's deployed standard `8888` Mini App preview token for the chosen supported chain. It remains the preview/E2E proof token only, not a production CA restriction, and it passes the same deployed-ERC20 validation as an explicitly supplied token.
...
4. For a default Vault UI, edit only the four package files under `src/vaults/my-vault`: `Component.tsx`, `manifest.json`, `VaultABI.ts`, and `i18n.json`. A mode-less 7777 Vault UI or token-scoped 8888 Mini App declaring `three-r3f-v1` may instead add only recursively statically reachable files allowed by that capability profile.
...
- remote images inside Vault source; immutable Vault-specific images must use `IpfsImage` from `@/src/ui` with a static image/directory CID. A dynamic in-directory NFT path is allowed only with a static `validationPath` sample verified by `vault:check`
- component-owned Vault V2 ABI/media resolution, metadata fetching, or `tokenURIBase` concatenation; use `NftMetadataImage` with `sdk` and token id so the shared runtime reads `Vault.nft()` and `NFT.tokenURI(tokenId)` across mode 0/1/2
...
For Vault V2 contract-selected media, use `<NftMetadataImage sdk={sdk} tokenId={tokenId} ... />`. This universal image path needs no project-supplied Vault V2 ABI: the runtime owns the minimal `Vault.nft()` and `NFT.tokenURI(tokenId)` ABIs. It reads the final token URI instead of letting the component inspect `tokenURIBase` or append `.json`; mode 0/1 data JSON is decoded locally and mode 2 IPFS/HTTPS metadata uses the host-owned resolver with DNS, redirect, timeout, size, MIME, and SVG safety limits. Render a paginated/visible token subset rather than mounting an unbounded collection. Project `VaultABI.ts` is still used for separate business methods such as mint, sell, pricing, or ownership queries.
