...
- component-owned Vault V2 metadata fetching or `tokenURIBase` concatenation; use `NftMetadataImage` with `sdk`, a Vault-derived NFT address, and token id so the shared runtime reads `NFT.tokenURI(tokenId)` across mode 0/1/2
...
For Vault V2 contract-selected media, use `<NftMetadataImage sdk={sdk} nftAddress={nftAddress} tokenId={tokenId} ... />`. The runtime reads the final `NFT.tokenURI(tokenId)` instead of letting the component read `tokenURIBase` or append `.json`; mode 0/1 data JSON is decoded locally and mode 2 IPFS/HTTPS metadata uses the host-owned resolver with DNS, redirect, timeout, size, MIME, and SVG safety limits. Render a paginated/visible token subset rather than mounting an unbounded collection.
