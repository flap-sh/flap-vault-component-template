...
For `three-r3f-v1` Mini App scaffolds, `--token` is optional when the project has no test token: the scaffold selects Flap's deployed standard `8888` Mini App preview token for the chosen supported chain. This fallback is only for the 8888 Mini App surface. A 7777 Vault UI must provide an explicit real deployed `7777` proof token. The token remains preview/E2E proof only, not a production CA restriction, and it passes the same deployed-ERC20 validation as an explicitly supplied token.
