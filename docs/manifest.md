...
Vault V2 NFT metadata is not a developer-declared endpoint or project-supplied ABI path. Use `NftMetadataImage` with the SDK and token id. The runtime owns the minimal ABI calls to `Vault.nft()` and `NFT.tokenURI(tokenId)`, and the host-owned resolver handles external IPFS/HTTPS metadata; do not declare or fetch `tokenURIBase` from `Component.tsx`.
