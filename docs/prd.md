...
| ABI policy | Done | `vault:check` warns when standard ERC20 methods are copied into `VaultABI.ts`, blocks human-readable ABI strings unless wrapped with `parseAbi([...])` from `viem`, and blocks object-typed `readContract` generics for ABI methods with multiple return values. |
