...
Endpoint declarations must be a single HTTPS URL string or an array of HTTPS URL strings. Host-relative fetches, non-HTTPS URLs, WebSocket URLs, IPFS/Arweave links, and embedded data URL media are blocked inside Vault source by default.
...
The source zip must include `flap-vault-package.json`, generated by `yarn vault:package <folder-name>`. Workbench validation should reject any manually assembled zip that is missing the marker, has a wrong package kind/version, has an unexpected file list, or has mismatched SHA-256 hashes.
...
It validates the package marker, package kind/version, exact file list, metadata consistency, schema entry, and SHA-256 hashes. It is a local acceptance proxy; the Flap Artifact Workbench still owns production validation and review.
...
| Script-generated package marker | Done | `vault:package` writes `flap-vault-package.json` and file hashes for Workbench rejection of manual zips. |
| Workbench-side package verifier | Done | `yarn vault:verify-package <zip>` checks marker, package kind/version, exact file list, metadata, and hashes. |
...
| Checker regression selftest | Done | `yarn vault:check:selftest` verifies critical blocking rules for invalid folder names, CA policy, duplicate bindings, endpoint declarations, endpoint-prefix escapes, hidden relative fetches, IPFS-style resources, dynamic imports, ERC20 ABI drift, registration formatting, unregister, and scaffold/check/package/verify positive flow. |
