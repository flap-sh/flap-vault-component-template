...
For oracle traffic, the current template preview uses a sibling same-origin route at `/api/runtime/oracle/{oracleId}`. That route now includes built-in defaults for the example oracle flow, so the public template works without user env setup. If a host/runtime later needs reviewed upstream URLs, allowlisted params, or auth headers for additional oracle ids, keep those in the host integration layer rather than inside the public Vault source package.
