...
9. Use `yarn vault:scaffold <folder-name> --chain 56 --factory 0x... --token 0x...` for new factory-scoped Vault packages, or `--vault 0x... --token 0x...` for no-factory packages. The `--token` value must be a real deployed ERC20 test token ending in `7777`; collect the final real mainnet factory binding early. In factory mode `tokenAddresses` is not the production CA restriction.
