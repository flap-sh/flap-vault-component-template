...
Contract calls should normally target runtime addresses: `context.vaultAddress`, `context.tokenAddress`, and `context.factoryAddress`. `Component.tsx` must not contain contract-address literals, even when an address is declared under `match.bindings[].externalContracts`; that declaration is review metadata, not runtime injection. A token or NFT address read directly from the current Vault by a static getter may be used downstream when the read has an explicit `Address` result type, for example `await sdk.readContract<Address>({ contract: "vault", address: context.vaultAddress, abi: vaultAbi, functionName: "basket" })`. Dynamic operational modules must remain behind Vault proxy methods.
