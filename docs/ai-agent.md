...
- `three-r3f-v1` supports two explicit surfaces: a mode-less Vault UI with only 7777 proof tokens, or the existing token-scoped 8888 Mini App. A 7777 artifact may use factory, single-Vault, or token bindings, but keeps the default shell, host risk status before the 3D visual, four-file rules except for profile-reachable 3D files, and no Mini App audio/full-height/displayTitle exemptions. Start from `src/vaults/three-r3f-example`, keep every file reachable through static imports from `Component.tsx`, and expose `data-flap-3d-state` plus `data-flap-3d-renderer`. Do not pass a relative string such as `"./assets/model.glb"` directly to `useGLTF` or another loader; import the asset first and pass the imported URL.
