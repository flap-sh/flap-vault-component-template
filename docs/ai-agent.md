...
Robinhood supports two proof paths. A token-scoped package may bind its real `7777`/`8888` token directly on Robinhood mainnet chain `4663`; that token is both the runtime scope and the E2E proof input. A factory- or Vault-scoped package may instead use a real deployed Robinhood Testnet token on chain `46630` (`https://rpc.testnet.chain.robinhood.com/rpc`). The standard chain `46630` proof token address is still TBD. Until a real address is supplied, leave this as an explicit open item and do not put a fabricated placeholder into `manifest.json`; `vault:check`, E2E packaging, and Workbench intake must remain blocked.
