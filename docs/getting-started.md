...
Vault V2 contract-selected NFT media does not use that static-CID shortcut. Read `nft()` from the Vault, then render `NftMetadataImage` with `sdk`, the returned `nftAddress`, and `tokenId`. It calls `NFT.tokenURI(tokenId)` and therefore follows mode 0/1 on-chain data JSON and mode 2 external JSON without inspecting `tokenURIBase` or constructing `.json` paths in UI code. External JSON and image bytes are resolved only through the host-owned metadata route with public-network, redirect, timeout, size, MIME, and SVG safety limits. Paginate token lists or mount only visible ids.
