...
Avoid external endpoints and external resources. If a special non-oracle endpoint is unavoidable, declare it in `manifest.json` as a single HTTPS URL string or an array of HTTPS URL strings; it will enter Flap review and must be approved before publish. Oracle usage is detected by `vault:check` and provisioned by the Flap Artifact Workbench/runtime, not declared in the manifest. Declaration does not guarantee approval, and undeclared URLs or host-relative endpoint calls are rejected.
