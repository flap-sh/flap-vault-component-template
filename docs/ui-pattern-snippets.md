...
- Third-party images or external media. If a Vault-specific immutable image is unavoidable, use `IpfsImage` from `@/src/ui`; for a decorative full-area background, use `IpfsBackground` from `@/src/ui`. Both require a static image/directory CID. `IpfsImage` alone may use a safe dynamic in-directory path with a static `validationPath` sample.
