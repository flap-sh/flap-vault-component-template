...
- For Vault V2 NFT art, use `NftMetadataImage` from `@/src/ui` with `sdk`, an NFT address read from the Vault, and `tokenId`. Do not read `tokenURIBase`, append `.json`, fetch metadata, or pass tokenURI/endpoint/src/imageUrl/CID/path props. The shared runtime resolves `NFT.tokenURI(tokenId)` across mode 0/1 data JSON and mode 2 IPFS/HTTPS metadata. Paginate or render only visible token ids; do not mount an unbounded collection.
