...
7. Gather inputs:
   ...
   - at least one manifest test token stored under any `match.bindings[].tokenAddresses`; prefer a testnet token, and do not treat factory-mode `tokenAddresses` as production CA restrictions
...
- Manifest match requires explicit runtime targets in `match.bindings`. Use `{chainId, factoryAddress}` for factory mode or `{chainId, vaultAddresses: [vaultAddress]}` for the core no-factory path. At least one binding must include `tokenAddresses` as the Workbench/E2E test token source; prefer a testnet token and also collect the final real mainnet factory binding early. The checker also accepts no-factory `tokenAddresses` and `vaultAddresses + tokenAddresses` mappings when Flap review/runtime supplies them. Do not use zero factory addresses; omit `factoryAddress` for no-factory mode. Optional `externalContracts` belongs inside the specific binding only.
