...
- Submit only the zip produced by `yarn vault:package {folder-name}`. The script writes format-version `6` `flap-vault-package.json`, E2E report v2, recursive source/asset hashes, and matching E2E summary fields. Workbench may read legacy format 5 only without `capabilities`; hand-made zips without the marker, proof, profile contract, or matching hashes are rejected.
...
- custom third-party image or external resource not controlled by Flap runtime/Artifact Workbench, except exact-host `https://bin.bnbstatic.com` images rendered through `BinanceImage`
