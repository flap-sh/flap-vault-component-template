...
- Only a token-scoped 8888 Mini App declaring `three-r3f-v1` may add recursively statically reachable profile-approved source, shaders, models, textures, environments, fonts, and controlled decoder assets inside the same folder. Do not add unreferenced files, path escapes, arbitrary npm packages, auxiliary documents, or sample data.
