...
- For Vault V2 NFT art, use `NftMetadataImage` from `@/src/ui` with only `sdk`, `tokenId`, and localized `alt`. No project image ABI or NFT address is needed: the shared runtime owns the minimal `Vault.nft()` and `NFT.tokenURI(tokenId)` ABIs and resolves mode 0/1 data JSON plus mode 2 IPFS/HTTPS metadata. Do not read `tokenURIBase`, append `.json`, fetch metadata, or pass ABI/nftAddress/tokenURI/endpoint/src/imageUrl/CID/path props. Paginate or render only visible token ids; do not mount an unbounded collection.
