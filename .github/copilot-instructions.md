...
- Declare at least one test token as `match.bindings[].tokenAddresses`; in no-factory mode this may also be the token-scoped target and may contain multiple token CAs. In factory mode this is not the production CA restriction. Ask for `caRestrictionMode` (`none`, `reserved`, or `verified`) and keep production CA restriction in Workbench/registry, not manifest fields.
...
- Fields `chainIds`, `id`, `owner`, `version`, `sdkVersion`, `actions`, `oracles`, `media`, `fallback`, `contracts`, `restrictTokenAddresses`, global `tokenAddresses`, or `caPolicy` in `manifest.json` — chain IDs, the required manifest test token, and optional external-contract lists live inside `match.bindings` entries; production CA restriction lives in Workbench/registry
