...
- Create new Vault packages with `yarn vault:scaffold <folder-name> --chain 56 --factory 0x... --token 0x...` for factory mode, or `yarn vault:scaffold <folder-name> --chain 56 --vault 0x... --token 0x...` for single-Vault mode without a factory. At least one manifest binding must include `tokenAddresses` for Workbench/E2E testing.
...
- Declare at least one test token as `match.bindings[].tokenAddresses`; in no-factory mode this may also be the token-scoped target and may contain multiple token CAs. Do not add global CA policy fields.
...
- Fields `chainIds`, `id`, `owner`, `version`, `sdkVersion`, `actions`, `oracles`, `media`, `fallback`, `contracts`, `restrictTokenAddresses`, global `tokenAddresses`, or `caPolicy` in `manifest.json` — chain IDs, the required manifest test token, and optional external-contract lists live inside `match.bindings` entries
